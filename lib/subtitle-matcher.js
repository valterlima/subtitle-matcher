	...
	async.parallel(
		...
		function getMatches(err, results){
			var matches = results[0].map(function(movie){
				var m = stringSimilarity.findBestMatch(movie, results[1]);
				return {
					movieFileName: movie, 
					subtitleFileName: m.bestMatch.target,
			...
			(function moveFiles(matches){
				...
				matches.forEach(function(item){
					...
					if (config.renameSubtitle){
						var newSubtitleFilename = subtitleRenamer(item.movieFileName, item.subtitleFileName);
						console.log('Renamed Subtitle:', item.subtitleFileName, newSubtitleFilename)
					}
					else{
						var newSubtitleFilename = item.subtitleFileName;
					}
